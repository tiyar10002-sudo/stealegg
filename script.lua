...
ProximityPromptService.PromptButtonHoldBegan:Connect(function(prompt, player)
    if player == o and tostring(prompt) == "CarryAreaEgg" then
...
d4=function(e,y,...)
    -- Trigger ProximityPrompt di telur target (logika DiceHub)
    if e then
        for e,r in ipairs(e:GetDescendants())do
    ...
    -- AUTO PICK UP: Scan semua ProximityPrompt "CarryAreaEgg" di map
    -- Ini yang bikin telur jatuh / kena guard / kena trap langsung diambil
    pcall(function()
        ...
        for _, prompt in ipairs(workspace:GetDescendants()) do
            if prompt:IsA("ProximityPrompt") and tostring(prompt) == "CarryAreaEgg" then
                local part = prompt.Parent
                if part and part:IsA("BasePart") then
                    local dist = (part.Position - hrp.Position).Magnitude
                    -- Kalo telur jatuh deket, langsung teleport & pick up
                    if dist > 5 and dist < 100 then
...
local function jM(...)
    ...
    local t=Instance.new ( "TextLabel" )t.Size =UDim2.new ( 1 , -28 , 0 , 16 )t.Position =UDim2.new ( 0 , 14 , 0 , 94 )t.BackgroundTransparency = 1 t.Text = "Initializing Turok Hub..." t.TextColor3 =Color3.fromRGB ( 130 , 140 , 165 )t.TextSize = 11 t.Font =Enum.Font.Gotham t.TextXAlignment =Enum.TextXAlignment.Left t.AutoLocalize = false t.Parent =r task.spawn (function(...)
        for y= 1 , 100 , 1 do
            if not e.Parent then
                break
            end
...
local function oM(...)
    ...
    local l=Instance.new ( "TextLabel" )l.Size =UDim2.new ( 1 , -90 , 0 , 22 )l.Position =UDim2.new ( 0 , 12 , 0 , 6 )l.BackgroundTransparency = 1 l.Text = "Turok Hub (Fallback)" l.TextColor3 =B l.TextSize = 14 l.Font =Enum.Font.GothamBold l.TextXAlignment =Enum.TextXAlignment.Left l.AutoLocalize = false l.Parent =U
